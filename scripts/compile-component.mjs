...
if (!input || !output || extra.length) {
  console.error('Usage: node scripts/compile-component.mjs [--primitives] input.js output.component.wasm');
  process.exitCode = 1;
} else {
  try {
    const ir = lower(buildSemantic(fs.readFileSync(input, 'utf8'), { dynamic }));
    const component = packageCommand(emitCoreWasm(ir, { target: 'command-adapter' }));
