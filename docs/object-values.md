...
See [the Phase 5 work log](phase-5-work-log.md) for complete baseline results. Prototype mutation, built-in prototypes, mutable captures and general callable values, exceptions, and additional built-ins remain separate migrations. The bounded captured-arrow subset is described in [closure-values.md](closure-values.md).
